...
def send_manychat_reply(subscriber_id, text, platform):
    ...
    logger.info(f"🔍 [SEND] Payload: {json.dumps(payload, indent=2)}")

    try:
        r = requests.post(url, json=payload, headers=headers, timeout=20)
        r.raise_for_status()  # رفع الاستثناء في حالة حدوث خطأ HTTP
        logger.info(f"📤 [SEND] Message delivered → {subscriber_id}")
        # تسجيل الاستجابة
        logger.info(f"📬 [RESPONSE] {r.text}")
    except requests.exceptions.HTTPError as err:
        logger.error(f"❌ [SEND] HTTPError: {err}")
        logger.error(f"❌ [SEND] Response: {r.text}")
    except Exception as e:
