...
# --- تم حذف دالة send_telegram_message القديمة لدمجها مباشرة ---
...
# ==============================================================================
# منطق Telegram (Webhook) - الإصدار النهائي مع الحل المدمج
# ==============================================================================
...
async def handle_any_message(update, context):
    ...
    if not message:
        logger.info("✅ التجاهل: التحديث لا يحتوي على رسالة يمكن معالجتها.")
        return

    chat_id = message.chat.id
    user_name = message.from_user.first_name
    
    # --- الحل النهائي: استخراج business_connection_id بأمان ---
    business_connection_id = getattr(update, 'business_connection_id', None)

    if business_connection_id:
        logger.info(f"📥 [Telegram Business] رسالة من: {chat_id} عبر الاتصال {business_connection_id}")
    else:
        logger.info(f"📥 [Telegram Direct] رسالة مباشرة من: {chat_id}")

    try:
        # محاولة إرسال "يكتب الآن" باستخدام business_connection_id
        ...
        logger.warning(f"⚠️ فشل إرسال chat action (يمكن تجاهله): {e}")
    ...
    if message.text:
        logger.info(f"💬 نوع الرسالة: نص. المحتوى: '{message.text}'")
        ...
        logger.info(f"❓ نوع الرسالة: غير مدعوم حاليًا. سيتم تجاهل المحتوى.")

    if content_for_assistant and not reply_text:
        reply_text = ask_assistant(content_for_assistant, chat_id, user_name)

    # --- منطق الإرسال المدمج والمحسّن ---
    if reply_text:
        try:
            logger.info(f"📤 جاري إرسال الرد إلى {chat_id} باستخدام business_connection_id: {business_connection_id}")
            await context.bot.send_message(
                chat_id=chat_id, 
                text=reply_text,
                business_connection_id=business_connection_id  # <-- هذا هو السطر الحاسم
            )
...
# --- ربط الـ Handlers ---
all_messages_handler = MessageHandler(filters.ALL, handle_any_message)
