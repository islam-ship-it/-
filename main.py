...
async def check_business_connection(bot_instance):
    """
    يتحقق من حالة اتصال البوت بحساب الأعمال.
    """
    try:
        business_connection = await bot_instance.getBusinessConnection()
        print(f"🔗 حالة اتصال حساب الأعمال: {business_connection}", flush=True)
        if business_connection.is_enabled:
            print("✅ البوت متصل بحساب أعمال تيليجرام ونشط.", flush=True)
        else:
            print("⚠️ البوت غير متصل بحساب أعمال تيليجرام أو غير نشط.", flush=True)
    except Exception as e:
        print(f"❌ خطأ أثناء التحقق من اتصال حساب الأعمال: {e}", flush=True)
...
def run_telegram_bot():
    ...
    loop = asyncio.get_event_loop()
    loop.run_until_complete(check_business_connection(bot))
